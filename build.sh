...
usage()
{
    echo "Usage: $0 [BuildArch] [BuildType] [clean] [verbose] [coverage] [cross] [clangx.y] [ninja] [configureonly] [skipconfigure] [skipnative] [skipmscorlib] [skiptests] [cmakeargs] [bindir] [testsonly] [priority <N>] [gcstresslevel <N>] [sequential] [ilasmroundtrip]"
    ...
    echo "testsonly - build only 'tests' subdirectory"
...
build_managed_tests()
{
    ...
    if [ $? -ne 0 ]; then
        echo "Failed to build managed tests."
        exit 1
    fi
...
build_tests()
{
    ...
    # Native tests are already created in build_Coreclr().
...
__SkipCoreCLR=0
__SkipMSCorLib=0
# The default value of __SkipManagedTests is false.
# You can enable this value using testsonly option.
__SkipManagedTests=1
__CleanBuild=0
__VerboseBuild=0
__SignTypeReal=""
__CrossBuild=0
